        ...
        if (isMining) {
            ...
            if (txtAddress.Text != "") {
                if (txtUser.Text != "") {
                    SaveValues();
                    proc = new Process();
                    proc.StartInfo.FileName = Path.Combine(Path.GetDirectoryName(Application.ExecutablePath), "cpuminer-sse2.exe");
                    proc.StartInfo.Arguments = " -a power2b -o " + "stratum+tcp://power2b.eu.mine.zergpool.com:7445" + " -u " + txtUser.Text.Trim() + " -p c=SCC " + (txtParams.Text.Trim() != "" ? " " + txtParams.Text.Trim() : "");
